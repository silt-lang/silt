...
extension TriviaPiece {
  ...
  public func combined(with piece: TriviaPiece) -> TriviaPiece? {
    ...
    case let (.carriageReturns(s1), .carriageReturns(s2)):
      return .carriageReturns(s1 + s2)
    default: return nil
    }
  }
  ...
  public var length: Int {
    ...
    case .spaces(let n), .tabs(let n), .newlines(let n),
         .verticalTabs(let n), .formfeeds(let n), .carriageReturns(let n):
      return n
    case .comment(let s):
      return s.count
    }
...
extension Trivia {
  ...
  public var containsWhitespace: Bool {
    for piece in pieces {
      if case .spaces(_) = piece { return true }
      if case .tabs(_) = piece { return true }
      if case .newlines(_) = piece { return true }
      if case .carriageReturns(_) = piece { return true }
    }
    return false
  }
}
